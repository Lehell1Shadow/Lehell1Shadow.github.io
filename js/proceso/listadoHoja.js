...
$(document).ready(function () {
    ...
    $("#btnNuevo").click(function () {
        location.href = rootUrl + '/Procesos/Hoja';
    });

    $("#executive_list_filter").change(function () {
        var _default_Id = "-1";
        var executiveId = $("#executive_list_filter").val();
        ...
        $.ajax({
            ...
            var groupList = $("#group_list_filter");
            groupList.empty();

            $("<option />", {
                val: _default_Id,
                text: "Mostrar todos los promotores"
            }).appendTo(groupList);
    ...
    SetSelectedMenu('#menu_ListadoHojas');
});
...
function BuildTable(tableLanguage) {
    var table = $('#ajax-hoja-table').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: rootUrl + '/Procesos/Hoja/Get',
            type: 'POST',
            data: function (d) {
                d.sucursalId = $("#branch_id").val();
                d.ejecutivoId = $("#executive_list_filter").val();
        ...
        "buttons": [
            ...
            },
            {
                ...
                titleAttr: 'Borrar registro semanal',
                action: function () {
...
var deleteSheetModal = function (table) {
    ...
    if (dataRows.length > 0) {
        ...
        swal({
            title: "Eliminar",
            text: "Estás seguro(a) de eliminar el registro semanal " + data.hS_Serie + "-" + data.hS_Folio + "?",
            type: "info",
...
function DeleteHoja(id, table) {
    ...
    $.ajax({
        method: "DELETE",
        url: rootUrl + "/Procesos/Hoja/Delete",
        data: {
            "hojaId": id,
            "branchId": branchId,
            "userName": userName
